...
let exitCode = 0;
try {
  execFileSync(electron, [appFile], { cwd: root, env, stdio: 'ignore', timeout: 90_000 });
} catch (err) {
  exitCode = err.status ?? -1;
}

const raw = fs.existsSync(resultFile) ? fs.readFileSync(resultFile, 'utf8') : '';
let results = [];
try {
...
check('复制命令 · cli 含 cd + 相对调用', /cd "E:\\AI\\tools\\jq"[\s\S]*\.\\jq\.exe/.test(map.get('cli') || ''), map.get('cli')?.replace(/\n/g, ' ⏎ '));
check('复制命令 · script 为带引号路径', (map.get('script') || '').startsWith('"') && (map.get('script') || '').endsWith('"'));
