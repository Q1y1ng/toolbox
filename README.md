...
| 指标 | 数值 |
| --- | --- |
| 收录工具 | **246**（curated 70 · 开始菜单 176） |
| 其中「我的项目」 | **10 个**（E:\AI 下的项目，每项一句简介 + 一键打开项目目录） |
...
- **每条都有简介**：curated 条目写在清单里；开始菜单条目由 `data/startmenu-blurbs.json`
  三级匹配生成 —— **名称精确匹配 → 目标路径正则 → 分类兜底**（实测 176 条里只剩 1 条落到兜底）。
- **「我的项目」分类**：把 `E:\AI` 下的项目（Starstate / launch-center / pi-standalone-gui /
  AI Chat / token-monitor / exam-pilot / RandomDrawer / _inventory / 工具箱目录 + 本面板）
  各做成一张卡，一句简介 + `📂 打开目录`。
