...
import type { CategoryMeta, Payload, Tool, ToolCategory, ToolboxState } from './types';
...
const CATEGORY_RULES: { re: RegExp; cat: ToolCategory }[] = [
  { re: /\b(steam|epic|ea app|rockstar|paradox|faceit|plain craft|spt\.|star ?map|mumu|perfectworld|5e对战|三角洲|鹰角|launcher|game)\b/i, cat: 'game' },
  { re: /(visual studio|pycharm|git |git$|node\.js|docker|unity|qgis|osgeo|python|anaconda|spyder|idle \(|postgres|pgadmin|psql|wsl|nsight|cuda|windows kits|developer|command prompt|powershell|performance analyzer|performance recorder|gpuview|application verifier|debuggable package|stack builder|java|jdk|flutter)/i, cat: 'dev' },
  { re: /(7-zip|gpu-z|图吧工具箱|afterburner|riva tuner|rivatuner|nexus|sideloadly|爱思助手|todesk|lenovo|联想|legion|machenike|disk|task manager|registry|event viewer|services|compmgmt|msconfig|msinfo32|dfrgui|cleanmgr|resource monitor|performance monitor|system information|administrative tools|iscsi|hyper-v|odbc|print management|recoverydrive|security configuration|firewall|memory diagnostics|speech recognition|steps recorder|character map|remote desktop|media player|wordpad|livecaptions|magnify|narrator|on-screen keyboard|voiceaccess|app cert kit|sysinternals|everything|crystaldiskinfo|wiztree|ventoy|rufus|memclean)/i, cat: 'system' },
  { re: /(photoshop|lightroom|premiere|acrobat|audition|after effects|illustrator|黑magic|blackmagic|epson|musescore|obs|剪映|网易云音乐|listen1|potplayer|foobar|vlc|音频|音乐|视频|相机)/i, cat: 'media' },
  { re: /(微信|qq|oopz|迅雷|网盘|123|夸克|百度|to ?desk|clash|加速器|输入法|邮箱|电脑管家|koook|kook|钉钉|飞书|telegram|discord)/i, cat: 'utility' },
];

function categorize(name: string, target: string | undefined, fallback: ToolCategory = 'app'): ToolCategory {
...
const DEFAULT_STATE: ToolboxState = { favorites: [], hidden: [], recent: [], categoryOverrides: {} };
...
export class Registry {
  ...
  load() {
    const curated = readJsonOrNull<{ tools: Tool[] }>(this.p('curated-tools.json'));
  ...
  private fromStartMenu(scan: ScanResult | null, seen: { paths: Set<string>; names: Set<string>; targets: Set<string> }): Tool[] {
  ...
  private fromPortable(scan: ScanResult | null, curatedTools: Tool[]): Tool[] {
    ...
    const curatedPaths = curatedTools.map((t) => (t.dir || t.path).toLowerCase());
    const isCovered = (portableDir: string) => {
      const d = portableDir.toLowerCase().replace(/[\\/]+$/, '');
      return curatedPaths.some((p) => p === d || p.startsWith(`${d}\\`) || p.startsWith(`${d}/`));
    };
    const out: Tool[] = [];
    const uncovered: string[] = [];
    for (const d of scan?.portable?.tools || []) {
      if (isCovered(d.dir)) continue;
      const exe = d.executables.find((e) => e.path.toLowerCase().endsWith('.exe'));
